...
api_open_weather_key = "2eafd2f7d930b529cdca7e8e4c363f05"
forecast_data = None
...
def create_faw_table():
    for i in range(0, NUM_STAGE):
        ...
        faw_table[1].append(int((BASE_TEMPERATURE - faw_table[0][i]) / (x * BASE_TEMPERATURE + y)))
        if 0 < i < NUM_STAGE - 1:
...
def get_temperature(query_time):
    if (query_time > date.today()):
        delta = int((query_time - date.today()).days)
        if (delta >= 30):
            print("Out of data range")
            sys.exit()
        return forecast_data.get_temperature(query_time)
    else:
        try:
...
class FAWPrediction:
    ...
    def calculate_mode_regression(self):
        ...
        for i in range(age, NUM_STAGE):
            dev_time = calculate_dev_time(i, get_temperature(str(dev_day)))
            dev_day += datetime.timedelta(dev_time)
